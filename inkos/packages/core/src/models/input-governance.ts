...
export const ContextSourceSchema = z.object({
  ...
  layer: z.enum(["hard_constraints", "plot_commitments", "relevant_memory", "recent_narrative", "style_guidance"]).optional(),
  confidence: z.number().min(0).max(1).optional(),
  updatedAt: z.string().datetime({ offset: true }).optional(),
  importance: z.number().int().min(0).max(100).optional(),
  trust: z.enum(["trusted", "untrusted_content"]).optional(),
});
...
export const ContextLayersSchema = z.object({
  hard_constraints: z.array(ContextSourceSchema).default([]),
  plot_commitments: z.array(ContextSourceSchema).default([]),
  relevant_memory: z.array(ContextSourceSchema).default([]),
  recent_narrative: z.array(ContextSourceSchema).default([]),
  style_guidance: z.array(ContextSourceSchema).default([]),
});
...
export type ContextPackage = {
  ...
  readonly selectedContext: ContextSource[];
  readonly layers?: z.infer<typeof ContextLayersSchema>;
