...
describe("StateManager", () => {
  ...
  describe("ensureControlDocuments", () => {
    ...
    it("fails closed instead of bootstrapping Runtime authority from Markdown", async () => {
      ...
      await expect(manager.ensureRuntimeState(bookId, 3)).rejects.toThrow(
        /Automatic Markdown bootstrap was removed in Phase 8/,
      );
      await expect(readFile(join(manager.stateDir(bookId), "manifest.json"), "utf-8"))
