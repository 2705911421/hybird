...
describe("runtime-state-store memory helpers", () => {
  ...
  it("rejects Markdown-only state instead of bootstrapping authority projections", async () => {
    ...
    await expect(loadRuntimeStateSnapshot(bookDir)).rejects.toMatchObject({ code: "ENOENT" });
  });
  ...
  it("rejects persisted duplicate summary chapters in structured runtime state", async () => {
    ...
    await expect(loadRuntimeStateSnapshot(bookDir)).rejects.toThrow(/duplicate_summary_chapter/);
  });

  it("rejects persisted hooks with empty type instead of repairing authority state", async () => {
    ...
    await expect(loadRuntimeStateSnapshot(bookDir)).rejects.toThrow(/at least 1 character/);
  });
});
