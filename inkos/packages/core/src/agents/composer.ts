...
export async function composeGovernedChapter(input: ComposeChapterInput): Promise<ComposeChapterOutput> {
  ...
  const legacyProvider = new LegacyTruthContextProvider(() => collectSelectedContext(
    storyDir,
    input.plan,
  ...
  const runtimeClient = input.storyRuntimeClient ?? (runtimeConfig.mode !== "legacy"
    ? new StoryRuntimeClient({
  ...
  const selection = await selectContextProvider({
    ...
    runtime: runtimeClient ? new StoryRuntimeContextProvider(runtimeClient) : undefined,
    request: {
      book: input.book,
    ...
    runtimeDir,
    fallbackOnUnavailable: runtimeConfig.fallbackOnUnavailable,
  });
  ...
  return {
    ...
    ruleStackPath,
    tracePath,
    ...(selection.shadowDiffPath ? { shadowDiffPath: selection.shadowDiffPath } : {}),
  };
}
